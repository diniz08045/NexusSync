...
def check_login_attempts(user_id, ip_address=None, max_attempts=5, window_minutes=15):
    ...
    most_recent = (
        LoginAttempt.query.filter(
            LoginAttempt.user_id == user_id, LoginAttempt.successful == False
        )
    ...
    if most_recent:
        ...
        if datetime.utcnow() < block_end_time:
            remaining_seconds = int(
                (block_end_time - datetime.utcnow()).total_seconds()
            )
    ...
    if is_blocked:
        logger.warning(
            f"Login blocked - User ID: {user_id}, IP: {ip_address}, "
...
def log_security_event(event_type, user_id=None, details=None):
    ...
    if user_id and event_type in suspicious_events:
        try:
            ...
            db.session.commit()
            logger.info(
                f"Created security notification for user_id {user_id}: {event_type}"
            )
...
def detect_suspicious_patterns(user_id, ip_address=None, user_agent=None):
    ...
    recent_attempts = (
        LoginAttempt.query.filter(
            LoginAttempt.user_id == user_id, LoginAttempt.timestamp > time_window
        )
        .order_by(LoginAttempt.timestamp.desc())
        .all()
    )
    ...
    max_hour = (
        max(hourly_attempts.items(), key=lambda x: x[1])
        if hourly_attempts
        else (None, 0)
    )
    ...
    if (
        max_hour[1] > 10
        and max_hour[1] > sum(hourly_attempts.values()) / len(hourly_attempts) * 2
    ):
        suspicious_patterns.append(
            {
